...
web_public_cache_start('product', 600);
...
if ($pdo) {
    try {
        ...
        if ($variant) {
            ...
            if ($legacySeries) {
                header('Location: series.php?slug=' . rawurlencode((string)$legacySeries['slug']), true, 302);
                exit;
            }
        }
    } catch (Throwable $e) {
...
if ($variant && $series && empty($prettyModelV71865) && ($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'GET') {
...
if (!$variant || !$series) {
    ...
    ?><!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><base href="/"><title>Product not found | Artdon Lighting</title><link rel="stylesheet" href="assets/css/artdon_home.css?v=6.8.4">
...
<main class="product-variant-page">
  ...
  <section class="variant-hero variant-hero-layout-split variant-layout-b-sheet <?= $dimensionImage!==''?'has-dimension':'no-dimension' ?> <?= $photometricImages?'has-photometric':'no-photometric' ?>">
    ...
    <div class="variant-actions variant-layout-b-actions"><a href="contact.php?product=<?= rawurlencode((string)$variant['name']) ?>&model=<?= rawurlencode((string)$variant['model_code']) ?>">Get a quote</a><?php if($downloadCount>0): ?><a href="#technical-files">Technical files</a><?php endif; ?><a href="product_pdf.php?slug=<?= rawurlencode((string)$variant['slug']) ?>" target="_blank" rel="noopener">PDF / Print</a><button type="button" class="artdon-share-open-v25" data-artdon-share-modal-open data-share-url="<?= web_e($shareUrl) ?>" data-share-title="<?= web_e($shareTitle) ?>" data-share-text="<?= web_e($shareText) ?>">Share link</button></div>
  </section>
  <?php else: ?>
  <section class="variant-hero variant-hero-layout-<?= web_e($detailLayout) ?> <?= $dimensionImage!==''?'has-dimension':'no-dimension' ?> <?= $photometricImages?'has-photometric':'no-photometric' ?>">
    ...
    <div class="variant-copy">
      ...
      <div class="variant-actions"><a href="contact.php?product=<?= rawurlencode((string)$variant['name']) ?>&model=<?= rawurlencode((string)$variant['model_code']) ?>">Get a quote</a><?php if($downloadCount>0): ?><a href="#technical-files">Technical files</a><?php endif; ?><a href="product_pdf.php?slug=<?= rawurlencode((string)$variant['slug']) ?>" target="_blank" rel="noopener">PDF / Print</a><button type="button" class="artdon-share-open-v25" data-artdon-share-modal-open data-share-url="<?= web_e($shareUrl) ?>" data-share-title="<?= web_e($shareTitle) ?>" data-share-text="<?= web_e($shareText) ?>">Share link</button></div>
    </div>
