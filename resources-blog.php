...
$defaultCategories = artdon_resource_blog_categories();
$categorySettings = [];
...
<main class="rb-page">
  ...
  <nav class="rb-tabs-wrap" aria-label="Blog categories">
    <div class="rb-tabs">
      ...
      <a class="rb-view-all" href="<?= artdon_v710_e(trim((string)($pageContent['view_all_articles_url'] ?? '')) ?: '#lighting-knowledge') ?>"><?= artdon_v710_e(trim((string)($pageContent['view_all_articles_text'] ?? '')) ?: 'VIEW ALL ARTICLES') ?> <span>→</span></a>
    </div>
  </nav>

  <?php foreach ($categories as $key => $label): $items = array_values($grouped[$key] ?? []); if (!$items) continue; $settings = $categorySettings[$key] ?? []; ?>
