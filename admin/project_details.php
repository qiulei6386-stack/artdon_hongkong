...
<section class="pd-admin">
  ...
  <form class="pd-page-hero-tool" action="project_action.php" method="post" enctype="multipart/form-data">
    ...
    <input type="hidden" name="slug" value="<?= web_e((string)$project['slug']) ?>">
    <input type="hidden" name="action" value="save_list_settings">
    <div><strong>项目页顶部横幅</strong><span>对应前台 <code>project.php</code> 的第一张大图；单独保存，不会改动任何项目卡片图片。</span></div>
    <div class="pd-page-hero-fields"><?php pd_admin_image('横幅图片', 'hero_image', 'project_page_hero_upload', $projectPageHeroImage, 'banners'); ?><label class="field"><span>图片 ALT</span><input type="text" name="hero_image_alt" value="<?= web_e((string)($projectPageSettings['hero_image_alt'] ?? 'Artdon lighting projects')) ?>" placeholder="Projects page banner"></label></div>
